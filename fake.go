...
func FakeShoppingBehavior() string{
	rand.Seed(time.Now().UTC().UnixNano()) // always seed random!

	events := make([]string, 0)

	authChooser, _ := wr.NewChooser(
		wr.Choice{Item: "success", Weight: 90},
		wr.Choice{Item: "failed", Weight: 10},
	)

	channelChooser, _ := wr.NewChooser(
		wr.Choice{Item: "女装", Weight: 60},
		wr.Choice{Item: "食品", Weight: 10},
		wr.Choice{Item: "男装", Weight: 5},
		wr.Choice{Item: "儿童", Weight: 10},
		wr.Choice{Item: "宠物", Weight: 13},
		wr.Choice{Item: "健康", Weight: 2},
	)

	cartChooser, _ := wr.NewChooser(
		wr.Choice{Item: "addCart", Weight: 8},
		wr.Choice{Item: "deleteCart", Weight: 2},
	)
	...
	for i := 0; i < 2000; i++ {
		...
		//鉴权100次
		result := authChooser.Pick().(string)
		authTemplate := "name=%s|action=%s|result=%s"
		auth := fmt.Sprintf(authTemplate, username, "login", result)
		if i<=100 {
			events = append(events, auth)
		}

        // 浏览2000次
		result = channelChooser.Pick().(string)
		browserTemplate := "name=%s|action=browser|channel=%s|cost=%d"
		browser := fmt.Sprintf(browserTemplate, username, result, gofakeit.Number(10,6000))
		events = append(events, browser)

		// 购物车40次
		cartTemplate := "name=%s|action=%s|item=%s"
		result = cartChooser.Pick().(string)
		cart := fmt.Sprintf(cartTemplate, username, result, gofakeit.SSN())
		if i< 40 {
			events = append(events, cart)
		}

		if i < 20 {
			orderNo := gofakeit.SSN()
			itemId := gofakeit.SSN()
			// 订单
			orderTemplate := "name=%s|action=%s|item=%s|orderNo=NO-%s|amount=%d"
			order := fmt.Sprintf(orderTemplate, username, result, itemId,orderNo ,
				gofakeit.Number(100,1000))

			// 支付
			result = authChooser.Pick().(string)
			payTemplate := "name=%s|action=%s|item=GOOD-%s|orderNo=NO-%s|result=%s"
			pay := fmt.Sprintf(payTemplate, username, "pay", itemId ,orderNo, result)

			// 物流
			expressTemplate := "name=%s|action=%s|orderNo=NO-%s"
			express := fmt.Sprintf(expressTemplate, username, "express", orderNo)

			events = append(events, order)
			events = append(events, pay)
			events = append(events, express)
		}
